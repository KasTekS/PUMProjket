...
public class GameR( private val binding: FragmentGalleryBinding)  {
    ...
    fun checkForWin(board: Array<CharArray>): String {
        ...
        if (board[0][0] == board[1][1] && board[1][1] == board[2][2] && board[0][0] != '_') {
            ...
            gameOver = true
            if(board[0][0].toString()==player1)
            return "wygrywa gracz" + player2
            else return "wygrywa gracz" + player1
    ...
    fun playerMove(board: Array<CharArray>, button: Button, x: Int, y: Int, player:String): Array<CharArray> {
        if (!gameOver) {
            if (button.isEnabled == true) {
                ...
                button.text = player
                board[x][y] = player[0]
                binding.stan.text =  checkForWin(board)
                var tab: Array<Int>? = null
                if(!gameOver) {

                    tab = computerMove(board, player1)
                    val x = tab?.get(0)
                    val y = tab?.get(1)
                    board[x!!][y!!] = player1[0]
                    binding.stan.text =  checkForWin(board)
        ...
        if (!gameOver) {
            if (button.isEnabled == true) {
                ...
                if(!gameOver) {
                    ...
                    tab = computerMoverand(board, player1)
                    val x = tab?.get(0)
                    val y = tab?.get(1)
                    board[x!!][y!!] = player1[0]
                    binding.stan.text =  checkForWin(board)
